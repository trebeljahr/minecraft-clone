...
class Chunk {
  ...
  load() {
    ...
    const { needsPropagation, voxels } = JSON.parse(
      fs.readFileSync(path.join(storage, `${x}_${z}.json`))
    );
    ...
    for (let x = 0; x < size; x += 1) {
      for (let y = 0; y < maxHeight; y += 1) {
        for (let z = 0; z < size; z += 1) {
          const { type, color } = generator.terrain(
            offset.x + x,
            y,
            offset.z + z
          );
  ...
  removeLight(x, y, z, key = "light") {
    const { fields, getVoxel, maxHeight, maxLight, voxelNeighbors } = Chunk;
    const { chunk, cx, cz } = this.get(x, z);
  ...
  update({ x, y, z, color = { r: 0, g: 0, b: 0 }, type }) {
    const {
      fields,
      getVoxel,
      maxHeight,
  ...
  meshSubChunk(subchunk) {
    const { getVoxel, fields, maxHeight, maxLight, size } = Chunk;
    const {
      world: {
        generator: { types },
      },
