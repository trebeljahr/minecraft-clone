...
class Voxels extends Object3D {
  ...
  update({ chunk, heightmap, geometries }) {
    ...
    ["opaque", "transparent"].forEach((key) => {
      const { color, light, position, uv } = geometries[key];
  ...
  static updateHeightmap({ chunk, geometry, heightmap }) {
    ...
    for (let i = 0; i < count; i += 4) {
      if (uv.getY(i) === 0) {
        ...
        for (let j = 0; j < 4; j += 1) {
          ...
          aux.y = Math.max(
            aux.y,
            offsetY + Math.ceil(position.getY(i + j) / 8)
          );
